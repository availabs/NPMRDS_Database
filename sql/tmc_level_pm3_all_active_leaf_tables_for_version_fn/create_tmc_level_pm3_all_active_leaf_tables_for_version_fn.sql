...
CREATE OR REPLACE FUNCTION tmc_level_pm3_all_active_leaf_tables_for_version_fn (
    state                VARCHAR(2),
    year                 INTEGER,
      ...
      WHERE (
        ...
        (
          CASE WHEN (tmcLevelPM3CalcVer IS NOT NULL)
            ...
            THEN (pg_class.relname ~ ('_\d{4}_v' || tmcLevelPM3CalcVer || '$'))
            ELSE true
          END
        )
      )
...
CREATE OR REPLACE FUNCTION tmc_level_pm3_all_active_leaf_tables_for_version_fn (
  ...
  RETURNS SETOF VARCHAR
  AS $body$
    SELECT tmc_level_pm3_all_active_leaf_tables_for_version_fn(
      state,
      year,
      npmrdsVer::NPMRDS_VERSION_TYPE,
      tmcLevelPM3CalcVer
