...
CREATE OR REPLACE FUNCTION public.qa_table_diff(
    state       TEXT,
    version_a   TEXT,
    version_b   TEXT
  )
...
COMMENT ON FUNCTION public.qa_table_diff(state TEXT, version_a TEXT, version_b TEXT) IS '
  This FUNCTION compares two tables.
';


CREATE OR REPLACE FUNCTION public.qa_table_columns_diff(
    state       TEXT,
    version_a   TEXT,
    version_b   TEXT
  )
  ...
  AS $$
    SELECT DISTINCT
      ...
      FROM public.qa_table_diff(state, version_a, version_b)
      ORDER BY 1
    ;
  $$ LANGUAGE SQL
;
