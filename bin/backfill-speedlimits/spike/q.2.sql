...
DROP INDEX IF EXISTS spike_endpt_geographies_gix;
  ...
  SELECT
    ...
    FROM (
      SELECT
          ...
          adj_tmc,
          RANK() OVER (
            PARTITION BY tmc ORDER BY dist, random()
          ) AS rank,
          dist
        FROM (
          SELECT
...
SELECT
    ...
    percentile_disc(array[0, 0.25, 0.5, 0.75, 0.8, 0.9, 1]) WITHIN GROUP (ORDER BY ABS(tmc_avg_speedlimt - adj_speedlimit)) AS diff_dist
