--  BEGIN;

--  DROP TABLE IF EXISTS spike_null_speedlimt_tmcs;
--  CREATE TABLE IF NOT EXISTS spike_null_speedlimt_tmcs AS
  --  SELECT tmc
    --  FROM ny.tmc_attributes
    --  WHERE (avg_speedlimit IS NULL)
--  ;

--  COMMIT;


--  BEGIN;

--  DROP TABLE IF EXISTS spike_know_speedlimit_tmcs_sample;
--  CREATE TABLE IF NOT EXISTS spike_know_speedlimit_tmcs_sample AS
  --  SELECT tmc
    --  FROM ny.tmc_attributes
    --  WHERE (avg_speedlimit IS NOT NULL)
    --  ORDER BY RANDOM()
    --  LIMIT 1000
--  ;

--  COMMIT;


--  BEGIN;

--  DROP TABLE IF EXISTS spike_endpt_geographies;
--  CREATE TABLE IF NOT EXISTS spike_endpt_geographies AS
  --  SELECT DISTINCT
      --  tmc,
      --  Geography(
        --  ST_StartPoint(line)
      --  ) AS pt_a,
      --  Geography(
        --  ST_EndPoint(line)
      --  ) AS pt_b,
      --  f_system
    --  FROM (
      --  SELECT
          --  tmc,
          --  (ST_Dump(wkb_geometry)).geom AS line,
          --  attr.f_system
        --  FROM ny.tmc_attributes AS attr
          --  INNER JOIN inrix_shapefile USING (tmc)
    --  ) AS sub_multilines_decomposed
--  ;


--  CREATE INDEX IF NOT EXISTS spike_endpt_geographies_gix ON spike_endpt_geographies USING GIST (pt_a, pt_b);

--  COMMIT;


BEGIN;

DROP TABLE IF EXISTS spike_adj_tmcs;
CREATE TABLE IF NOT EXISTS spike_adj_tmcs AS
  SELECT
      tmc,
      adj_tmc,
      rank,
      dist
    FROM (
      SELECT
          tmc,
          adj_tmc,
          RANK() OVER (
            PARTITION BY tmc ORDER BY dist
          ) AS rank,
          dist
        FROM (
          SELECT
              t.tmc AS tmc,
              a.tmc AS adj_tmc,
              (
                (
                  levenshtein(
                    regexp_replace(
                      regexp_replace(t.tmc, 'P', '+'), 
                      'N',
                      '-'
                    ),
                    regexp_replace(
                      regexp_replace(a.tmc, 'P', '+'), 
                      'N',
                      '-'
                    )
                  )
                ) +
                --  (10 * LEAST(ST_Distance(t.pt_a, a.pt_a), ST_Distance(t.pt_a, a.pt_b))) +
                (15 * ABS(t.f_system - a.f_system))
              ) AS dist
            FROM spike_endpt_geographies AS t
              INNER JOIN spike_endpt_geographies AS a ON (
                ST_DWithin(t.pt_a, a.pt_a, 10)
                OR
                ST_DWithin(t.pt_a, a.pt_b, 10)
              )
            WHERE (
              (t.tmc <> a.tmc)
              AND
              (t.tmc IN (SELECT tmc FROM spike_know_speedlimit_tmcs_sample))
              AND
              (a.tmc NOT IN (SELECT tmc FROM spike_know_speedlimit_tmcs_sample))
              AND
              (a.tmc NOT IN (SELECT tmc FROM spike_null_speedlimt_tmcs))
            )
          ) AS sub_adjacents
    ) AS sub_ranked
    WHERE (sub_ranked.rank = 1)
;

COMMIT;


DROP TABLE IF EXISTS spike_backfilled_speedlimts;
CREATE TABLE spike_backfilled_speedlimts
  AS
    SELECT 
        a.tmc,
        b.tmc AS adj_tmc,
        a.avg_speedlimit tmc_avg_speedlimt,
        b.avg_speedlimit AS adj_speedlimit
      FROM spike_adj_tmcs s
        INNER JOIN tmc_attributes AS a USING (tmc)
        INNER JOIN tmc_attributes AS b ON (b.tmc = s.adj_tmc)
 ; 

SELECT
    avg(ABS(tmc_avg_speedlimt - adj_speedlimit)) AS avg_diff,
    percentile_disc(array[0, 0.25, 0.5, 0.75, 1]) WITHIN GROUP (ORDER BY ABS(tmc_avg_speedlimt - adj_speedlimit)) AS diff_dist
  FROM spike_backfilled_speedlimts
;
